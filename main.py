...
from utils.rag import query_collection
ingest_data()
...
def format_products(products):
    ...
    for product in products:
        name = product.get("Product Name", "N/A")
        category = product.get("category", "N/A")
        price = product.get("Price", "N/A")
        description = product.get("Product Description", "N/A")
        details = product.get("Product Details", "N/A")
        vibe = product.get("Vibe", "N/A")

        formatted = f"""Product Name: {name},
                    category: {category},
                    Price: {price},
        ...
        formatted_list.append(
            formatted.strip())  # remove leading/trailing whitespace
...
async def chat(session_id: str, request: Request):
    ...
    results = query_collection("products", user_input, 10)
    ...
    top_docs, top_metadata = (
        zip(*doc_metadata_pairs) if doc_metadata_pairs else ([], [])
    )
    ...
    if session_id not in sessions or is_session_expired(sessions[session_id]["last_active"]):
        sessions[session_id] = {
            ...
            "last_active": now,
            "products_list": format_products(top_metadata)
        }
    else:
